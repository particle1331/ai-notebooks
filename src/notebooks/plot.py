"""
High-level plotting API for ML/DL/LLM notebooks.

Wraps matplotlib with a builder/chained interface and clean defaults:
no top/right spines, dotted grid, sans-serif font stack.
...
Usage::
    ...
    p[0].line(train_losses, label="train", color="C0", smooth=50)
    p[0].line(valid_losses, x=valid_steps, label="valid", color="C1")
    p[0].labels(x="step", y="loss")
    p[1].scatter(x, y, color="C0").regression_line()
    p.legend(loc="below")
    p.show()
"""
...
def _to_numpy(data: _ArrayLike) -> np.ndarray | None:
    """Convert lists, torch tensors, or arrays to numpy.

    Returns ``None`` if *data* is ``None``.  Handles the common
    ``.detach().cpu().numpy()`` chain for torch tensors so callers
        ...
        *,
        ...
        if shared:
            ...
            for panel in self._panels:
                ...
                for hi, li in zip(h, panel_labels):
                    if li not in seen:
                        handles.append(hi)
                        labels.append(li)
                        seen.add(li)
