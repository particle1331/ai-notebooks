...
```html
<video src="./img/flet/greeting.mp4" autoplay loop muted
...
All section index pages (`index.ipynb`) follow this pattern:

1. **`# Section Title`** (H1 in first markdown cell)
2. Introductory prose paragraph(s)
3. Optional `<br>` spacer
4. **`## About This Series`** -- audience, goals
5. **`## Course Notebooks`** -- markdown table with links and topic summaries
6. **`## What You'll Build Understanding Of`** -- bullet list of learning outcomes
7. **`## Prerequisites`**
8. **`## How to Read This Series`** -- guidance for different reader profiles
9. **`## References`** (optional, with arXiv links)
10. Cross-links between sections: `[Deep Learning Foundations](../deep/index.ipynb)`
11. Footnotes: `[^1]: explanation text`
